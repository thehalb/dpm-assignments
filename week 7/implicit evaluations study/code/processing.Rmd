...
data_processed <- data_processed_before_exclusions |>
  mutate(exclude_participant = case_when(tolower(age) == "test" ~ "exclude",
                                         ...
                                         is.na(like) | is.na(prefer) | is.na(positive) ~ "exclude",
                                         TRUE ~ "include"
                                        )
)
```

#Check for correct trials in the AMP task

```{r}
# Calculate the total number of trials per subject in the AMP task
amp_trials_count_with_practice_instructions <- data_amp_raw |>
  group_by(subject) |>
  summarize(total_trials = n()) #82

#Counting of trials per subject in the AMP task without practice and instructions
amp_trials_count_without_practice_instructions <- data_amp_raw |>
  filter(blockcode != "practice", trialcode != "instructions") |>
  count(subject) # 72

#excluding people with another trial number as 72
amp_trials_count_without_practice_instructions <- data_amp_raw|>
  group_by(subject) |>
